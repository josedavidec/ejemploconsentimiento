        ...
        <table className="clientes-table">
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr>
                <td colSpan="8" style={{textAlign: 'center', padding: '40px', color: '#dc3545'}}>
                  Error: {error}
                </td>
              </tr>
              ...
              filteredClientes.map(cliente => {
                const diasRestantes = getDiasRestantes(cliente.fecha_inicio, cliente.dias_sanitizacion);
                ...
                return (
                  <tr key={cliente.id}>
                    ...
                    <td>
                      <span className={`status-badge ${cliente.estado.toLowerCase().replace(' ', '-')}`}>
                        {cliente.estado}
                      </span>
                    </td>
                    <td>
                        ...
                        <div className="progress-bar-container">
                          <div 
                            ...
                            style={{
                              ...
                              backgroundColor: getProgressColor(cliente.progreso, cliente.estado)
                            }}
                          ></div>
                        </div>
                        <span className="progress-text">{cliente.progreso}%</span>
                      </div>
                    </td>
                    <td>
                      <span className={`days-remaining ${diasRestantes <= 7 ? 'urgent' : diasRestantes <= 15 ? 'warning' : ''}`}>
                        {diasRestantes} días
                      </span>
                    </td>
                    <td>
        ...
        <div className="modal-overlay" onClick={closeModal}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2>{modalType === 'add' ? 'Agregar Cliente' : 'Editar Cliente'}</h2>
            ...
            <form onSubmit={handleSubmit} className="modal-form">
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) => setFormData({...formData, dias_sanitizacion: parseInt(e.target.value)})}
                  min="1"
                  max="365"
                  required
                />
              ...
              <div className="modal-actions">
                ...
                <button type="submit" className="btn-primary" disabled={isSubmitting}>
                  {isSubmitting ? 'Guardando...' : (modalType === 'add' ? 'Agregar' : 'Guardar')}
                </button>
              </div>
            </form>
          </div>
